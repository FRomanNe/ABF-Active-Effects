...
import { NoneWeaponCritic, WeaponCritic ,resistanceEffectCheck,damageCheck,
  ABFFoundryRoll,ABFSettingsKeys,ABFConfig} from "../../animabfConnector.js";
...
class CombatAttackDialog extends FormApplication {
  constructor(attacker, defender, hooks, options = {}) {
    ...
    //Module
    const withoutRoll = this.attackerActor.getFlag(
      "animabf",
      "withoutRoll"
    );
    if(withoutRoll) this.modalData.attacker.withoutRoll = true;

    const criticalBonus = this.attackerActor.getFlag(
      "animabf",
      "criticalBonus"
    );
    if(criticalBonus)
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      ...
      height: null,
      resizable: true,
      template: Templates.combat.attack.main,
  ...
  activateListeners(html) {
    ...
    html.find(".send-attack").click(async () => {
      ...
      if (typeof damage !== "undefined") {
        ...
        //Module
        if (weapon !== void 0 && criticSelected !== NoneWeaponCritic.NONE 
          && (criticSelected == weapon?.system.critic.secondary.value || weapon?.system.critic.other?.includes(criticSelected))) {
        ...
        let counterFlagBonus = this.attackerActor.getFlag("animabf","counterattackBonus");
        if(isNaN(counterFlagBonus)) counterFlagBonus = 0;
        const counterAttackBonus = this.modalData.attacker.counterAttackBonus??0 + counterFlagBonus;
  ...
  getData() {
    ...
    if (combat.unarmed) {
      ...
      //Module
      ui.weaponHasSecondaryCritic = (weapon.system.critic.secondary.value !== NoneWeaponCritic.NONE || weapon.system.critic.other?.length>0);
